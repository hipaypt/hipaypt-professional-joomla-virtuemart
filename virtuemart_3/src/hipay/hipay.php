...
/**
 * @version $Id: hipay.php,v 1.0 2015/10/01 19:33:57 
 *
...
class plgVmPaymentHipay extends vmPSPlugin {
	...
	function plgVmConfirmedOrder ($cart, $order) {
        ...
        $cleanXML.='<currency>'.$account["payment_currency"].'</currency>';
        ...
        $cleanXML.='<logo_url>'.$account["account_storelogo"].'</logo_url>';
	...
	function getPaymentAccount($method){
		...
		foreach ($payment_params as $key => $value) {
			...
			$account[$value_temp[0]] = str_replace('"','',$value_temp[1]);
		}
		return $account;
	}
